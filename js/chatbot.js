...
var KB = [
  ...
  {k:['best time','best season','when to visit','when should','افضل وقت','أفضل وقت','وقت الزيار','وقت زيار','متى ازور','متى أزور','متى الزيار','موسم','weather','طقس','الطقس','حراره','الحراره'],
  ...
  {k:['best place','top place','must see','must visit','افضل اماكن','أفضل اماكن','افضل الاماكن','أفضل الأماكن','افضل مكان','أفضل مكان','احسن مكان','احسن اماكن','وش اشوف','وش أشوف','وين اروح','وين أروح','اماكن','أماكن','معالم','recommend','رشح','توصي','اقتراح','اقترح'],
...
function reply(msg){
  ...
  var best = null;       // أفضل تطابق وجدناه
  var bestLen = 0;       // طول أطول كلمة مفتاحية طابقت
  for(var i=0;i<KB.length;i++){
    for(var j=0;j<KB[i].k.length;j++){
      var kw = KB[i].k[j];
      // لو الرسالة تحتوي الكلمة المفتاحية، وهذي الكلمة أطول من أفضل تطابق سابق
      if(m.indexOf(kw) !== -1 && kw.length > bestLen){
        bestLen = kw.length;
        best = KB[i];
      }
    }
  }
  if(best) return isAr() ? best.ar : best.en;
